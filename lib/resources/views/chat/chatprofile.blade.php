		...
		<section class="content container-fluid">
                       ...
                       <div class="dropdown">
						  ...
						  <div class="dropdown-content">
						  	@foreach($account as $value)
						  	 @if($value->cate_id == $row->id)
						      <a href="javascript:void(0);"><img style="border-radius: 50%; margin-right: 8px; border: solid 1px #ccc;width: 30px;height: 30px;" src="{{ $value->image }}">{{ $value->name }}</a>
	                ...
	                @endforeach
					  ...
					  .threadChat__btn-is-friend {
							...
							.dropdown-content a:hover {background-color: #44a1ead9;}
				       ...
				       <div>
						...
						<ul class="nav nav-tabs" style="color: #6e68ff; background-color: #ecf0f5; margin-bottom:0;">
							...
							<li class="nav-item active">
								...
								<a href="javascript:void(0);" class="nav-link active homet" onclick="activetab(this,'#home')" style="float: left;">Tất cả 
							            ...
							            <div class="flx-cell flx flx-col" style="position: relative; width: 0px; z-index: 5;">
							                ...
							                <div id="contact-search" class="ovf-hidden web">
							                    ...
							                    <span id="composeBtn" style="position: relative;">
							                        ...
							                        <div tabindex="1" aria-hidden="true" class="clickable btn icon-add-group" data-translate-title="STR_CREATE_CHAT_GROUP" title="Tạo nhóm chat"><i class="fa fa-icon-add-group icon" data-toggle="modal" data-target="#creategroup"></i></div>
							                        ...
							                        <div>
							                            ...
							                            <div style="padding-top: 6px; margin-bottom: 2px; display: flex; justify-content: space-between;">
							                                ...
							                                <div style="margin-right: -5px;">
							                                    ...
							                                    <div class="filter-tag-title filter-button-section showCreatetag" data-translate-title="STR_CREATE_LBL" title="Tạo thẻ tag" data-toggle="modal" data-target="#createtag"><i class="setting-icon fa fa-tag-add" style="top: -1px;"></i></div>
							                        ...
							                        <div class="threadChat__title  ">
							                            ...
							                            <div id="groupNameInput--change">
							                                ...
							                                <div class="title header-title group-title user">
							                                    ...
							                                    <div class="truncate content" id="nameContent">A10 Huy</div>

							                                    <i class="fa fa-edit edit-icon clickable getNameZalo"></i>
							                                    <div class="dropdown-menu editName">
							                                    	{{-- <input type="text" name="newName" placeholder="Tên gợi nhớ."><i class="fa fa-check-circle" id="addNewName" aria-hidden="true" style="margin-left: 5px; color: #0068ff;font-size: 15px;"></i> --}}
							                                    </div>
...
<script language="javascript">
      $( document ).ready(function() {
		     var locationv2 = window.location.href;
		      ...
		      if(locationv2 == 'https://zalov2.phanmemninja.com/chat/chatprofile'){
	  ...
	  var mautag1 = document.getElementById("btn1");
             ...
             setInterval(function myTimer() {
             	...
             	for (var i = 0; i < profilecheck.length; i++) {
	              	setTimeout(function timer(){
	                   var cookie = $('.profile_'+profilecheck[n]).data('cookie');
					   var serectkey = $('.profile_'+profilecheck[n]).data('env');
					   var urlchat = $('.profile_'+profilecheck[n]).data('urlchat');
					   var datav2 = '{"page": 1, "count": 100, "cookie":"'+cookie+'","serectkey":"'+serectkey+'","id_profile":"'+profilecheck[n]+'"}';
	     ...
	     function runAccount_zalo(ob){
			                ...
			                if(result.status == 200){
			                    for (var i = 0; i < result.data.length; i++) {
			                    	if(result.data[i].checkcookie != 1){
			                    		console.log(n);
			                    		// sendDataAccount(result.data[i].zalo_id);
			                    ...
			                    for (var i = 0; i < check.length; i++) {
			                    	...
			                    	setTimeout(function timer(){
			                    			if(check.checkcookie != 1){
			                    				var cookieSub = $('.profile_'+check[h]).data('cookie');
					                    		var serectkeySub = $('.profile_'+check[h]).data('env');
					                    		var urlchatSub =  $('.profile_'+check[h]).data('urlchat');
					                    		var dataSub = '{"cookie":"'+cookieSub+'","serectkey":"'+serectkeySub+'","urlchat":"'+urlchatSub+'","id_profile":"'+check[h]+'"}';
					                    		var datav2Sub = '{"page": 1, "count": 100, "cookie":"'+cookieSub+'","serectkey":"'+serectkeySub+'","id_profile":"'+check[h]+'"}';
	                                            
				                    		   socket.emit('messprofile',dataSub);
						                       socket.emit('getfriendnew',dataSub);

						                       socket.emit('getnamev2',datav2Sub);
			                    			}
                                 ...
                                 setInterval(function myTimer() {
			                    	...
			                    	setTimeout(function timer(){
			                    			...
			                    			if(check.checkcookie != 1){
			                    				...
			                    				var cookie1 = $('.profile_'+check[n]).data('cookie');
					                    		var serectkey1 = $('.profile_'+check[n]).data('env');
					                    		var urlchat1 =  $('.profile_'+check[n]).data('urlchat');
					                    		var id_profile1 = check[n];
		   ...
		   function addNewName(){
		      	var name =  $('.editName input[name="newName"]').val();
		   	    var id_profile = $('.submitAddName').data('profile');
		   	    var user = $('.submitAddName').data('user');
		   	    var cookie = $('.profile_'+id_profile).data('cookie');
		   	    var env = $('.profile_'+id_profile).data('env');
		   	    // var param = '{"friendId": "'+user+'", alias: "'+name+'","cookie":"'+cookie+'","env":"'+env+'"}';

		   	    var param = '{"friendId":"'+user+'","alias":"'+name+'","cookie":"'+cookie+'","env":"'+env+'"}';
                socket.emit('addnewname',param);
